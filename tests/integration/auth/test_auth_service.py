...
async def test_google_auth__login_exist_user(auth_service, get_db_session):
    query = insert(UserProfile).values(id=EXISTS_GOOGLE_USER_ID, email=EXISTS_GOOGLE_USER_EMAIL)
    code = "fake_code"
    ...
    async with get_db_session as session:
        ...
        login_user = (
            await session.execute(select(UserProfile).where(UserProfile.id == user_data.user_id))
        ).scalar_one_or_none()
...
async def test_base_login__success(auth_service, get_db_session):
    ...
    async with get_db_session as session:
        ...
        await session.flush()
        login_user = (
            await session.execute(select(UserProfile).where(UserProfile.username == username))
        ).scalar_one_or_none()
